...
class ParserInterface {
	...
	// TODO (#007): toggle targetRollsCritSuccess and targetRollsCritFailure externally — see issues/007-parser-interface-todos.md

	// Set up the parser with our custom random function
	initParser(){
		this.rollParser = new DiceRoller((rolls = this.rollsAsFloats) => {
			if(rolls.length > 0) {
				return rolls[this._externalCount++]
			} else {
	...
	// TODO (#007): this needs to return an object of rolls that need to be rolled again — see issues/007-parser-interface-todos.md
		...
		rollResults.forEach((group, groupId) => {
			...
			if(group.mods?.length > 0){
				...
				group.mods.forEach(mod => {
					// TODO (#007): handle each type of mod that would trigger a reroll — see issues/007-parser-interface-todos.md
					...
					switch(mod.type){
						...
						case "compound":
							...
							Object.entries(rollsCopy).forEach(([key, die]) => {
								...
								// TODO (#007): pass back die theme in reroll object — see issues/007-parser-interface-todos.md
								// TODO (#007): destructure die object and replace rollId while keeping other properties (e.g., scale) — see issues/007-parser-interface-todos.md
						...
						case "penetrate":
							...
							// TODO (#007): explosions beyond the first are being dropped; investigate and add tests (see issues/007-parser-interface-todos.md)
							Object.entries(rollsCopy).forEach(([key, die]) => {
								const max = die.sides
								const target = mod.target?.value?.value || max
								const op = mod.target?.mod || '='
  ...
  handleTargetCritFailure(finalResults = []){
    ...
    if(this.targetRollsCritFailure && finalResults.success !== null){
      this.handleTargetCritFailure(finalResults)
		...
		// after parse clear out instance state
		this._externalCount = 0
		this.rollsAsFloats = []

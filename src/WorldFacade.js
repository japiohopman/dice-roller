...
const defaultOptions = {
  ...
  delay: 10, // delay between dice being generated - 0 causes stuttering and physics popping
	...
	assetPath: '/assets/', // path to 'ammo', 'themes' folders and web workers
		...
		this.#DicePhysics.onmessage = (e) => {
			switch( e.data.action ) {
				case "init-complete":
					...
					break
				case "init-failed":
					console.warn('Physics worker reported init failure:', e.data && e.data.error)
					this.#dicePhysicsResolve()
					break
			}
    }
		this.#DicePhysics.onerror = (err) => {
			console.warn('Physics worker error event:', err)
			try { this.#dicePhysicsResolve() } catch (e) {}
		}
  ...
  async init() {
    ...
    if(this.#webgl_support){
      ...
      this.#dicePhysicsPromise = Promise.resolve()
		...
		// ensure physics worker is initialized before we accept roll requests
		if(this.#dicePhysicsPromise){
			// wait for physics init but don't hang forever — fall back after timeout
			let physicsInitOk = false
			try {
				physicsInitOk = await Promise.race([
					this.#dicePhysicsPromise.then(() => true),
					new Promise((resolve) => setTimeout(() => resolve(false), 3000))
				])
				if (!physicsInitOk) {
					console.warn('Physics worker did not initialize within timeout — continuing without physics')
				}
			} catch (err) {
				console.warn('Physics worker initialization failed:', err)
			}
		}

		// now that DiceWorld and physics worker are ready we can attach our callbacks
    ...
    if(this.#DicePhysics){
      ...
      this.#connectWorld()
		...
		await Promise.all(this.config.preloadThemes.map(theme => {
			return this.loadThemeQueue.push(() => this.loadTheme(theme));
		}));
		...
		this.boxControls = new BoxControls({
			themes: Object.keys(this.themesLoadedData),
			onUpdate: onUpdate,
		...
		this.boxControls.themeSelect.remove();
		this.boxControls.themeSelect = this.boxControls.gui.__folders.Rendering.add(this.boxControls.config, 'theme', Object.keys(this.themesLoadedData)).onChange(this.boxControls.handleUpdate.bind(this.boxControls));
  ...
  add(notation, {theme = this.config.theme, themeColor = this.config.themeColor, newStartPoint = true} = {}) {
	...
	async #makeRoll(parsedNotation, collectionId){
		...
		parsedNotation.forEach(async notation => {
			...
			console.log("Dice Available in theme '" + theme + "':", this.themesLoadedData[theme]?.diceAvailable);
			console.log("Rolling dieType:", Number.isInteger(notation.sides) ? `d${notation.sides}` : notation.sides);
